...
// src/server/ai-client/client.ts
var clientMap2 = {};
...
// src/server/ai-client/index.ts
async function queryStream(aiName, key, query, attachments = [], messageResult = []) {
...
async function kimiQueryStream(aiName, key, query, messageResult = []) {
  const client = getClient(aiName, key);
  return await client.instance.chat.completions.create({
    model: client.model,
    messages: [
...
async function handleUpload(httpWrapper) {
  ...
  if (!token?.length) {
    ...
    httpWrapper.replyJson(400, { error: "Invalid request" });
    return;
  } else if (!import_node_fs.default.existsSync(data.filePath)) {
    httpWrapper.replyJson(400, { error: "File not found" });
    return;
  }
  const stream = import_node_fs.default.createReadStream(data.filePath);
