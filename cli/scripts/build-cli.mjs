...
function writeEnvFile() {
  ...
  // 将dts复制到模板目录
  // 递归删除
  fs.rmSync(path.join(__dirname, '..', '..', 'templates', 'node_modules', '@ddd-tool', 'domain-designer-core'), {
    recursive: true,
    force: true,
  })
  ...
  // 递归复制所有.d.ts文件
  copyFolderRecursive(
    path.join(__dirname, '..', '..', 'node_modules', '@ddd-tool', 'domain-designer-core'),
    path.join(__dirname, '..', '..', 'templates', 'node_modules', '@ddd-tool', 'domain-designer-core'),
    { pattern: /^.+\.d\.ts$/ }
  )
}
